 ...
 * sixteen hexes in slot order; the renderers implement the corrected parallax ladder
 * and the six palette rules. Embedded into index.html for atomic offline updates.
 */
(() => {
  "use strict";
  ...
  /* Corrected parallax ladder (brief section 2). */
  const RATES = { haze: 0.03, clouds: 0.05, far: 0.10, mid: 0.30, world: 1.0, fore: 1.7 };
  ...
  const ENVS = [
    {
      ...
      far(g, off) {
        g.repeat(off, 112, 60, x => {
          for (let i = 0; i < 3; i++) {
            const h = 62 + i * 26, bx = x + i * 38;
            g.px(bx, 372 - h, 32, h, "FAR-1");
            for (let y = 372 - h + 8; y < 366; y += 10) g.px(bx + 3, y, 26, 3, "FAR-2");
          }
        });
      },
      mid(g, off, t) {
        // Palms sway 1 px; the obstacle never animates.
        g.repeat(off, 118, 70, x => {
          const sway = g.reduced ? 0 : Math.round(Math.sin(t * 1.1 + x * .04));
          g.px(x + 28, 392, 5, FLOOR - 392, "MID-2");
          for (let i = 0; i < 4; i++) {
            const a = i * 1.57;
            g.px(x + 28 + Math.round(Math.cos(a) * 17) + sway, 384 + Math.round(Math.sin(a) * 7), 16, 6, "MID-1");
          }
          g.px(x + 74, 404, 30, FLOOR - 404, "MID-2");
          g.px(x + 70, 398, 38, 7, "MID-1");
          for (let i = 0; i < 5; i++) g.px(x + 71 + i * 8, 405, 4, 4, "TRIM", .5);
        });
      },
      ground(g, off) {
    ...
    },
    {
      ...
      clouds: [[0, 82, 0, .8]],
      far(g, off) {
        g.repeat(off, 168, 90, x => {
          for (let i = 0; i < 2; i++) {
            const bx = x + i * 84, peak = 306 + i * 14;
            for (let s = 0; s < 40; s++) {
              const hh = Math.round(Math.sin((s / 40) * Math.PI) * (56 - i * 12));
              g.px(bx + s * 2, peak + (56 - hh), 2, 372 - peak - (56 - hh), i ? "FAR-2" : "FAR-1");
            }
          }
        });
      },
      mid(g, off, t) {
        // Aqueduct rhythm is deliberately offset from the obstacle rhythm.
        g.repeat(off, 74, 40, x => {
          g.px(x, 360, 74, 12, "MID-1");
          g.px(x + 6, 372, 14, 46, "MID-1");
          g.px(x + 54, 372, 14, 46, "MID-1");
          g.px(x + 20, 372, 34, 8, "MID-2");
        });
        g.repeat(off, 92, 40, x => {
          const lean = g.reduced ? 0 : Math.round(Math.sin(t * .9 + x * .05));
          g.px(x + 12 + lean, 386, 8, FLOOR - 386, "MID-2");
          g.px(x + 10 + lean, 380, 12, 12, "MID-2");
        });
      },
      ground(g, off) {
        g.px(0, FLOOR, W, H - FLOOR, "GROUND");
    ...
    },
    {
      ...
      sky(g, t) {
        ...
        sky(g, t);
        for (let x = 0; x < W; x += 56) g.px(x, 0, 3, HAZE_TOP, "FAR-2", .45);
        for (let y = 24; y < HAZE_TOP; y += 46) g.px(0, y, W, 3, "FAR-2", .45);
      },
      glow: null,
      clouds: null,
      far(g, off) {
        g.px(0, 336, W, 30, "FAR-1");
        g.repeat(off, 18, 20, x => g.px(x + 6, 340, 5, 22, "FAR-2"));
        g.px(0, 332, W, 5, "FAR-2");
      },
      mid(g, off, t) {
        g.px(0, 392, W, 10, "MID-1");
        g.repeat(off, 128, 70, x => {
          g.px(x + 8, 402, 26, 22, "MID-2");
          g.px(x + 4, 396, 34, 7, "MID-1");
          for (let i = 0; i < 3; i++) g.px(x + 10 + i * 8, 388, 6, 9, "TRIM");
          // Chandelier on a long, slow period.
          const sway = g.reduced ? 0 : Math.round(Math.sin(t * .5 + x * .02));
          g.px(x + 78 + sway, 336, 3, 22, "MID-2");
          g.px(x + 70 + sway, 358, 20, 8, "MID-2");
          g.px(x + 74 + sway, 366, 12, 5, "SKY-GLOW", .6);
        });
      },
      ground(g, off) {
        g.px(0, FLOOR, W, H - FLOOR, "GROUND");
    ...
    },
    {
      ...
      far(g, off) {
        g.repeat(off, 148, 80, x => {
          for (let s = 0; s < 74; s++) {
            const hh = Math.round(Math.sin((s / 74) * Math.PI) * 34) + 8;
            g.px(x + s * 2, 372 - hh, 2, hh + 6, s % 3 ? "FAR-1" : "FAR-2");
          }
        });
      },
      mid(g, off, t) {
        g.px(0, 396, W, FLOOR - 396, "MID-1");
        g.repeat(off, 96, 50, x => {
          g.px(x, 396, 52, 6, "MID-2");
          g.px(x + 60, 404, 36, 5, "MID-2");
          // Sprinkler arcs, well clear of the play column.
          const p = g.reduced ? .5 : (Math.sin(t * 1.6 + x * .03) + 1) / 2;
          for (let i = 0; i < 7; i++) {
            const a = (i / 6) * Math.PI;
            g.px(x + 20 + Math.round(Math.cos(a) * 22), 396 - Math.round(Math.sin(a) * 14 * p), 2, 2, "TRIM", .6);
          }
        });
      },
      ground(g, off) {
    ...
    },
    {
      ...
      premise: "A rooftop run above a city at dusk - water towers, HVAC stacks, neon. Night-native, so the \"day\" ramp is dusk rather than noon.",
      ...
      readability: "Both ramps are dark, so the rim-lit OBST-EDGE does all the work. A dark world makes the warm rider glow, but this is the least tonal range of the six and neon invites clutter.",
      scores: { separation: "Fair", charm: "Medium", clutter: "High", themeRange: "Poor (dark only)" },
      dayLabel: "Dusk",
      nightLabel: "Night",
      exceptions: [
      ...
      clouds: [[0, 118, 0, 1]],
      far(g, off, t) {
        g.repeat(off, 126, 70, x => {
          for (let i = 0; i < 4; i++) {
            const h = 54 + ((i * 37) % 58), bx = x + i * 32;
            g.px(bx, 372 - h, 26, h, i % 2 ? "FAR-2" : "FAR-1");
            for (let wy = 372 - h + 6; wy < 366; wy += 8) {
              for (let wx = 0; wx < 3; wx++) {
                // Lit-window dither on a slow pseudo-random schedule.
                const seed = bx * .3 + wy * .7 + wx;
                const lit = g.reduced ? hash(seed) > .45 : hash(seed + Math.floor(t * .8) * .13) > .45;
                if (lit) g.px(bx + 4 + wx * 7, wy, 4, 4, "SKY-GLOW", .5);
              }
            }
          }
        });
      ...
      mid(g, off, t) {
        g.repeat(off, 142, 80, x => {
          g.px(x + 6, 388, 30, 26, "MID-1");
          g.px(x + 2, 382, 38, 7, "MID-1");
          for (let i = 0; i < 4; i++) g.px(x + 8 + i * 8, 414, 4, FLOOR - 414, "MID-2");
          // Billboard frames only: no legible text anywhere.
          g.px(x + 74, 356, 52, 3, "MID-2");
          g.px(x + 74, 392, 52, 3, "MID-2");
          g.px(x + 74, 356, 3, 39, "MID-2");
          g.px(x + 123, 356, 3, 39, "MID-2");
          g.px(x + 78, 360, 44, 31, "TRIM", .12);
          // Steam plumes rise from mid-layer stacks only.
          if (!g.reduced) for (let i = 0; i < 3; i++) {
            const p = wrap(t * .5 + i * .33, 1);
            g.px(x + 18, 382 - p * 26, 6, 5, "SKY-GLOW", (1 - p) * .3);
          }
        });
      },
      ground(g, off) {
    ...
    },
    {
      ...
      weather(g, off, t) {
        ...
        for (let i = 0; i < 2; i++) {
          const y = 46 + i * 26;
          const x = g.reduced ? i * 180 + 40 : wrap(t * 9 + i * 210, W + 200) - 100;
          g.px(x, y, 96, 2, "SKY-GLOW", .34);
          g.px(x + 96, y, 6, 2, "SKY-GLOW", .6);
        }
      },
      far(g, off) {
        g.repeat(off, 134, 70, x => {
          g.px(x, 344, 62, 28, "FAR-1");
          g.px(x, 340, 62, 5, "FAR-2");
          g.px(x + 76, 352, 40, 20, "FAR-2");
          g.px(x + 120, 322, 4, 50, "FAR-2");
        });
      },
      mid(g, off, t) {
        g.px(0, 398, W, FLOOR - 398, "MID-1");
        g.px(0, 398, W, 5, "MID-2");
        g.repeat(off, 116, 60, x => {
          g.px(x + 10, 404, 44, 6, "MID-2");
          // The windsock flips with the same wind cycle that drives the rider's hair.
          const wind = g.reduced ? 1 : Math.sign(Math.sin(t * 5.2)) || 1;
          g.px(x + 72, 372, 3, 26, "MID-2");
          for (let i = 0; i < 4; i++) g.px(x + 75 + wind * i * 5, 374 + i, 5, 5 - i, "TRIM", .8);
